...
class DocumentRepository extends BaseRepository implements DocumentRepositoryInterface
{
    ...
    public function getDocuments($attributes)
    {
        ...
        $query = Documents::select(['documents.id', 'documents.name', 'documents.type', 'documents.url', 'documents.createdDate', 'documents.description', 'categories.id as categoryId', 'categories.name as categoryName', DB::raw("CONCAT(users.firstName,' ', users.lastName) as createdByName")])
        ...
        if (isset($folderId)) {
            $folders = DocumentFolder::where('parent_id',$folderId)->get()->toArray();
        }else{
            $folders = DocumentFolder::where('parent_id',null)->get()->toArray();
        }

        $fileResults = $query->skip($attributes->skip)->take($attributes->pageSize)->get()->toArray();

        $results = array_merge($fileResults,$folders);
